...
export class ServerApp {
     ...
     */
    applyRoutes = (routeUrl: string, routing: Routing | Route, middlewares: Handler[] = [],): void => {
        switch (routing.isRoute) {
            case false:
                // eslint-disable-next-line no-param-reassign
                routing.url = routeUrl + (routing.url === '/' ? '' : routing.url);
                routing.childRoutes.forEach((childRoute) => {
                    this.applyRoutes(routing.url, childRoute, [...middlewares]);
                });
                break;
            case true:
                // eslint-disable-next-line no-param-reassign
                ...
                } else this.router[method](path,  [ ...middlewares, ...handlers]);
                break;
            default:
        }
    };
